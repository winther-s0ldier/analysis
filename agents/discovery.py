    ...
    if _discovery_agent_instance is None:
        ...
        _discovery_agent_instance = Agent(
            ...
            instruction=(
                ...
                "Discovers events that reliably precede session or process abandonment (>80% dropout rate after the event). "
                "Produces a ranked list of risk-level triggers with their dropout rates. "
                "STRONGLY preferred for event-log datasets where early-exit patterns are important.\n"
                "- `session_classification`: {\"entity_col\": <id>, \"event_col\": <event>, \"time_col\": <time>} — "
                "Classifies entities into behavioural archetypes based on journey depth, event diversity, and outcome signals. "
                "Reveals which engagement tiers exist and where the biggest volume of incomplete journeys occurs. "
                "STRONGLY preferred for event-log datasets.\n"
                ...
                "Kaplan-Meier curve measuring what fraction of sessions remain active at each successive event depth. "
                "Identifies the typical session 'half-life' and the depth at which most sequences terminate.\n\n"
                "NOTE: `user_journey_analysis` is the correct type for path or flow analysis. "
                "DO NOT use `path_analysis` — it is not a registered type.\n"
                "NOTE: `cohort_analysis` is the correct type for retention or repeat-engagement tracking. "
                "DO NOT use `retention_analysis` — it is not a registered type.\n"
                "NOTE: `trend_analysis` is the correct type for time-to-event or temporal patterns. "
